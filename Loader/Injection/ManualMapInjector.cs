...
namespace Injection {
    internal class ManualMapInjector {
        ...
        PIMAGE_DOS_HEADER GetDosHeader(IntPtr address) {
        ...
        PIMAGE_NT_HEADERS32 GetNtHeader(IntPtr address) {
        ...
        IntPtr RemoteAllocateMemory(uint size) {
            return Imports.VirtualAllocEx(hProcess,
                UIntPtr.Zero,
                new IntPtr(size),
        ...
        IntPtr AllocateMemory(uint size) {
            return Imports.VirtualAlloc(IntPtr.Zero, new UIntPtr(size),
                Imports.AllocationType.Commit | Imports.AllocationType.Reserve,
                Imports.MemoryProtection.ExecuteReadWrite);
        }

        IntPtr RvaToPointer(uint rva, IntPtr baseAddress) {
        ...
        bool InjectDependency(string dependency) {
            ...
            var result = Imports.WriteProcessMemory(hProcess, lpAddress, buffer, buffer.Length, out bytesWritten);

            if (result) {
                var hHandle =
                    Imports.CreateRemoteThread(hProcess, IntPtr.Zero, 0, procAddress, lpAddress, 0, IntPtr.Zero);
            ...
            Imports.VirtualFreeEx(hProcess, lpAddress, 0, Imports.FreeType.Release);

            return result;
        }

        IntPtr GetRemoteModuleHandleA(string module) {
            ...
            var pbi = (PPROCESS_BASIC_INFORMATION)Imports.HeapAlloc(hHeap, /*HEAP_ZERO_MEMORY*/ 0x8,
                new UIntPtr(dwSize));

            uint dwSizeNeeded;

            var dwStatus = Imports.NtQueryInformationProcess(hProcess, /*ProcessBasicInformation*/ 0, pbi.Address,
                dwSize, out dwSizeNeeded);
            ...
            if (dwStatus >= 0 && dwSize < dwSizeNeeded) {
                ...
                pbi = (PPROCESS_BASIC_INFORMATION)Imports.HeapAlloc(hHeap, /*HEAP_ZERO_MEMORY*/ 0x8,
                    new UIntPtr(dwSize));
                ...
                dwStatus = Imports.NtQueryInformationProcess(hProcess, /*ProcessBasicInformation*/ 0, pbi.Address,
                    dwSizeNeeded, out dwSizeNeeded);
            }
            ...
            if (dwStatus >= 0) {
                if (pbi.Value.PebBaseAddress != IntPtr.Zero) {
                    ...
                    if (Imports.ReadProcessMemory(hProcess, pbi.Value.PebBaseAddress + 12 /*peb.Ldr*/,
                        out pebLdrAddress, out dwBytesRead)) {
                        ...
                        do {
                            ...
                            if (!Imports.ReadProcessMemory(hProcess, new IntPtr(pLdrCurrentNode), out lstEntryAddress,
                                out dwBytesRead)) {
                                Imports.HeapFree(hHeap, 0, pbi.Address);
                            }
                            ...
                            Imports.ReadProcessMemory(hProcess, new IntPtr(lstEntryAddress) + /*BaseDllName*/ 0x2C,
                                out baseDllName, out dwBytesRead);

                            var strBaseDllName = string.Empty;

                            if (baseDllName.Length > 0) {
                                var buffer = new byte[baseDllName.Length];
                                Imports.ReadProcessMemory(hProcess, baseDllName.Buffer, buffer, out dwBytesRead);
                            ...
                            Imports.ReadProcessMemory(hProcess, new IntPtr(lstEntryAddress) + /*DllBase*/ 0x18,
                                out dllBase, out dwBytesRead);

                            Imports.ReadProcessMemory(hProcess, new IntPtr(lstEntryAddress) + /*SizeOfImage*/ 0x20,
                                out sizeOfImage, out dwBytesRead);
        ...
        IntPtr GetDependencyProcAddressA(IntPtr moduleBase, PCHAR procName) {
            var pFunc = IntPtr.Zero;
            IMAGE_DOS_HEADER hdrDos;
            IMAGE_NT_HEADERS32 hdrNt32;

            UIntPtr dwRead;
            Imports.ReadProcessMemory(hProcess, moduleBase, out hdrDos, out dwRead);

            if (!hdrDos.isValid) {
                return IntPtr.Zero;
            }

            Imports.ReadProcessMemory(hProcess, moduleBase + hdrDos.e_lfanew, out hdrNt32, out dwRead);
            ...
            if (expBase > 0) {
                ...
                Imports.ReadProcessMemory(hProcess, moduleBase + (int)expBase, expData.Address, (int)expSize,
                    out dwRead);

                var pAddressOfOrds =
                    (PWORD)(expData.Address + (int)expData.Value.AddressOfNameOrdinals - (int)expBase);

                var pAddressOfNames = (PDWORD)(expData.Address + (int)expData.Value.AddressOfNames - (int)expBase);

                var pAddressOfFuncs =
                    (PDWORD)(expData.Address + (int)expData.Value.AddressOfFunctions - (int)expBase);
                ...
                for (uint i = 0; i < expData.Value.NumberOfFunctions; i++) {
                    ...
                    if ((new PDWORD(procName.Address).Value <= 0xFFFF &&
                        ...
                        (new PDWORD(procName.Address).Value > 0xFFFF && pName.ToString() == procName.ToString())) {
                        pFunc = moduleBase + (int)pAddressOfFuncs[ordIndex];

                        if (pFunc.ToInt64() >= (moduleBase + (int)expBase).ToInt64() &&
                            pFunc.ToInt64() <= (moduleBase + (int)expBase + (int)expSize).ToInt64()) {
                            var forwardStr = new byte[255];
                            Imports.ReadProcessMemory(hProcess, pFunc, forwardStr, out dwRead);
        ...
        bool ProcessImportTable(IntPtr baseAddress) {
            var imageNtHeaders = GetNtHeader(baseAddress);
            ...
            if (imageNtHeaders.Value.OptionalHeader.ImportTable.Size > 0) {
                var imageImportDescriptor =
                    (PIMAGE_IMPORT_DESCRIPTOR)RvaToPointer(
                        imageNtHeaders.Value.OptionalHeader.ImportTable.VirtualAddress, baseAddress);
                ...
                if (imageImportDescriptor != null) {
                    for (; imageImportDescriptor.Value.Name > 0; imageImportDescriptor++) {
                        ...
                        if (imageImportDescriptor.Value.OriginalFirstThunk > 0) {
                            imageThunkData =
                                (PIMAGE_THUNK_DATA)RvaToPointer(imageImportDescriptor.Value.OriginalFirstThunk,
                                    baseAddress);

                            imageFuncData =
                                (PIMAGE_THUNK_DATA)RvaToPointer(imageImportDescriptor.Value.FirstThunk, baseAddress);
                        }
                        else {
                            imageThunkData =
                                (PIMAGE_THUNK_DATA)RvaToPointer(imageImportDescriptor.Value.FirstThunk, baseAddress);
                        ...
                        for (; imageThunkData.Value.AddressOfData > 0; imageThunkData++, imageFuncData++) {
                            ...
                            var bSnapByOrdinal = (imageThunkData.Value.Ordinal & /*IMAGE_ORDINAL_FLAG32*/ 0x80000000) !=
                                                 0;
                            ...
                            if (bSnapByOrdinal) {
                                ...
                                var imageImportByName =
                                    (PIMAGE_IMPORT_BY_NAME)RvaToPointer(imageFuncData.Value.Ordinal, baseAddress);
        ...
        bool ProcessDelayedImportTable(IntPtr baseAddress, IntPtr remoteAddress) {
            var imageNtHeaders = GetNtHeader(baseAddress);
            ...
            if (imageNtHeaders.Value.OptionalHeader.DelayImportDescriptor.Size > 0) {
                var imageDelayedImportDescriptor =
                    (PIMAGE_IMPORT_DESCRIPTOR)RvaToPointer(
                        imageNtHeaders.Value.OptionalHeader.DelayImportDescriptor.VirtualAddress, baseAddress);
                ...
                if (imageDelayedImportDescriptor != null) {
                    for (; imageDelayedImportDescriptor.Value.Name > 0; imageDelayedImportDescriptor++) {
                        ...
                        if (imageDelayedImportDescriptor.Value.OriginalFirstThunk > 0) {
                            imageThunkData =
                                (PIMAGE_THUNK_DATA)RvaToPointer(imageDelayedImportDescriptor.Value.OriginalFirstThunk,
                                    baseAddress);

                            imageFuncData =
                                (PIMAGE_THUNK_DATA)RvaToPointer(imageDelayedImportDescriptor.Value.FirstThunk,
                                    baseAddress);
                        }
                        else {
                            imageThunkData =
                        ...
                        for (; imageThunkData.Value.AddressOfData > 0; imageThunkData++, imageFuncData++) {
                            ...
                            var bSnapByOrdinal =
                                ((imageThunkData.Value.Ordinal & /*IMAGE_ORDINAL_FLAG32*/ 0x80000000) != 0);
        ...
        bool ProcessRelocation(uint imageBaseDelta, ushort data, PBYTE relocationBase) {
            var bReturn = true;
            PSHORT raw;
            PDWORD raw2;
            ...
            switch ((data >> 12) & 0xF) {
                case 1: // IMAGE_REL_BASED_HIGH
                    ...
                    Marshal.WriteInt16(raw.Address,
                        unchecked((short)(raw.Value + (uint)((ushort)((imageBaseDelta >> 16) & 0xffff)))));
        ...
        bool ProcessRelocations(IntPtr baseAddress, IntPtr remoteAddress) {
            var imageNtHeaders = GetNtHeader(baseAddress);
            ...
            if ((imageNtHeaders.Value.FileHeader.Characteristics & /*IMAGE_FILE_RELOCS_STRIPPED*/ 0x01) > 0) {
                ...
                if (relocationSize > 0) {
                    var relocationDirectory = (PIMAGE_BASE_RELOCATION)RvaToPointer(
                        imageNtHeaders.Value.OptionalHeader.BaseRelocationTable.VirtualAddress, baseAddress);
        ...
        uint GetSectionProtection(DataSectionFlags characteristics) {
            uint result = 0;
        ...
        bool ProcessSection(char[] name, IntPtr baseAddress, IntPtr remoteAddress, ulong rawData, ulong virtualAddress,
            ulong rawSize, ulong virtualSize, uint protectFlag) {
            UIntPtr lpNumberOfBytesWritten;
            uint dwOldProtect;

            if (
                !Imports.WriteProcessMemory(hProcess, new IntPtr(remoteAddress.ToInt64() + (long)virtualAddress),
                    new IntPtr(baseAddress.ToInt64() + (long)rawData),
                    new IntPtr((long)rawSize), out lpNumberOfBytesWritten)) {
                return false;
            }

            if (!Imports.VirtualProtectEx(hProcess, new IntPtr(remoteAddress.ToInt64() + (long)virtualAddress),
                new UIntPtr(virtualSize), protectFlag, out dwOldProtect)) {
                return false;
            }
        ...
        bool ProcessSections(IntPtr baseAddress, IntPtr remoteAddress) {
            var imageNtHeaders = GetNtHeader(baseAddress);
            ...
            var imageSectionHeader = (PIMAGE_SECTION_HEADER)(imageNtHeaders.Address + /*OptionalHeader*/ 24 +
            ...
            for (ushort i = 0; i < imageNtHeaders.Value.FileHeader.NumberOfSections; i++) {
                ...
                if (characteristics.HasFlag(DataSectionFlags.MemoryRead) ||
                    ...
                    ProcessSection(imageSectionHeader[i].Name, baseAddress, remoteAddress,
                        imageSectionHeader[i].PointerToRawData,
                        imageSectionHeader[i].VirtualAddress, imageSectionHeader[i].SizeOfRawData,
                        imageSectionHeader[i].VirtualSize, protection);
                }
            }
        ...
        bool ExecuteRemoteThreadBuffer(byte[] threadData, bool async) {
            ...
            var result =
                Imports.WriteProcessMemory(hProcess, lpAddress, threadData, threadData.Length, out bytesWritten);

            if (result) {
                var hHandle =
                    Imports.CreateRemoteThread(hProcess, IntPtr.Zero, 0, lpAddress, IntPtr.Zero, 0, IntPtr.Zero);
        ...
        bool CallEntryPoint(IntPtr baseAddress, uint entrypoint, bool async) {
        ...
        IntPtr LoadImageToMemory(IntPtr baseAddress) {
            var imageNtHeaders = GetNtHeader(baseAddress);
            ...
            var imageSectionHeader = (PIMAGE_SECTION_HEADER)(imageNtHeaders.Address + /*OptionalHeader*/
                                                              24 + imageNtHeaders.Value.FileHeader
            ...
            if (imageNtHeaders.Value.OptionalHeader.AddressOfEntryPoint > 0) {
                var dllEntryPoint = allocatedRemoteMemory.ToInt32() +
        ...
        GCHandle PinBuffer(byte[] buffer) => GCHandle.Alloc(buffer, GCHandleType.Pinned);

        void FreeHandle(GCHandle handle) {
        ...
        void OpenTarget() {
            hProcess = Imports.OpenProcess(process, Imports.ProcessAccessFlags.All);
        ...
        public IntPtr Inject(string file) => Inject(File.ReadAllBytes(file));
